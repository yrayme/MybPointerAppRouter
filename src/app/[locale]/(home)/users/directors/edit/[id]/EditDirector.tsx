...
const EditDirectors: React.FC<Props> = ({ data }) => {
  const router = useRouter();
  const { t } = useTranslation();
  const { countries } = useCountries();
  const { errors, control, register, handleSubmit, handleSubmitData, isLoading, isValid, watch } = useAddDirector(data);  
  ...
  return (
    <div>
      <div className='flex gap-4 items-center'>
        ...
        <p className='font-semibold text-lg'>{t("directors:add:edit")}</p>
      </div>
